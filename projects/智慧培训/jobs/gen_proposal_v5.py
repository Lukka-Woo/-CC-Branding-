...
# Update footer disclaimer text on regular pages
...
doc.add_data_table(
    headers=['席位类型', '适用对象', '核心权限', '人数'],
    data_rows=[
        ['A 类（完整席位）', '办公室人员 + 管理者', '学习 / 考试 / 积分 + 管理后台访问', '500 人'],
        ['B 类（轻量席位）', '一线员工',             '学习 / 考试 / 积分（仅学员端）',     '2,000 人'],
    ],
    col_widths_mm=[30, 40, 65, 20],
)

doc.add_heading('5.2  费用明细', level=2)
doc.add_data_table(
    headers=['项目', '规格 / 说明', '金额（不含税）'],
    data_rows=[
        ['A 类席位年费', '500 席 × 完整功能授权', '¥80,000'],
        ['B 类席位年费', '2,000 席 × 轻量学员端授权', '¥60,000'],
        ['一次性实施费', '需求确认 · 配置部署 · 培训交付', '¥38,000'],
        ['合计（第 1 年）', '含席位 + 实施', '¥178,000'],
        ['续费（第 2 年起）', '仅席位年费，无实施费', '¥140,000 / 年'],
    ],
    col_widths_mm=[40, 85, 30],
    first_col_bold=True,
)
...
OUTPUT  = os.path.join(_DOCS, 'Vesuvius_培训平台增强建议书_v5.docx')
